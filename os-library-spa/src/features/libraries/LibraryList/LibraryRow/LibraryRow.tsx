...
export default function LibraryRow({ library }: Props): ReactElement {
    const dispatch = useDispatch()
    const history = useHistory();

    const maxDownloads = useSelector(selectMaxDownloads)
    
    const { author, name, repository, downloads, uid, uri } = library;
    ...
    return (
        <tr className={classes.LibraryRow}>
            ...
            <td><button disabled={!uri} onClick={handleDetail}>🌎</button></td>
        </tr>
    )
}
